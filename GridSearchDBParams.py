        ...
        def runQueries(param):
            ...
            for query in queries:
                    try:
                            ...
                            # cursor = cnxn.cursor()
                            # cursor.execute(f"ALTER SYSTEM SET max_parallel_workers = {param};SELECT pg_reload_conf();")
                            # cursor.commit()
                            # cursor.close()

                            start = time.time()
                            cursor = cnxn.cursor()
                            #cursor.execute(f"SET statement_timeout = '{param}s'")
                            ...
                            pass
                    count += 1
            print(f"For {param} Total Query time : {str(sum(times))}")
            return sum(times)
...
tuned_params = {"intValue" : [2,4,6]}
#print(len(X_test),len(X_train))6
